...
Hero* loadSavedData(char* filename, int haveItems)
{
	...
	if(haveItems == 1)
	{
		for(i = 0; i < NUMCHOSENHEROES; i++)
		{
			heroes[i].heroSword = (Sword*)malloc(sizeof(Sword));
			fscanf(fp, "%s | %d | %d", heroes[i].heroSword->name, 
				&(heroes[i].heroSword->cost), &(heroes[i].heroSword->attack));
			if(heroes[i].heroSword->name[0] == 'N')
			{
		...
		for(i = 0; i < NUMCHOSENHEROES; i++)
		{
			...
			fscanf(fp, "%s | %d | %d", heroes[i].heroShield->name, 
				&(heroes[i].heroShield->cost), &(heroes[i].heroShield->defense));
			if(heroes[i].heroShield->name[0] == 'N')
			{
